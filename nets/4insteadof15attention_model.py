...
class AttentionModel(nn.Module):
    ...
    def _make_field_stats(self, templates):
        ...
        center = points.mean(dim=2)          # [B, N, 2]
        min_xy = points.min(dim=2).values    # [B, N, 2]
        max_xy = points.max(dim=2).values    # [B, N, 2]

        size_xy = max_xy - min_xy            # [B, N, 2]
