...
class EdgeBiasedMultiHeadAttentionLayer(nn.Module):
    """
    Transformer encoder layer with additive edge bias in self-attention.

    h:          [B, M, D]
    edge_bias:  [B, H, M, M], added to QK^T/sqrt(d_k) before softmax
    """
    ...
    def _apply_norm(self, norm, x):
        if self._use_batch_norm:
            B, M, D = x.size()
            return norm(x.reshape(B * M, D)).reshape(B, M, D)
        return norm(x)

    def forward(self, h, edge_bias=None):
        B, M, D = h.size()

        q = self.q_proj(h).view(B, M, self.n_heads, self.head_dim).transpose(1, 2)
        k = self.k_proj(h).view(B, M, self.n_heads, self.head_dim).transpose(1, 2)
        v = self.v_proj(h).view(B, M, self.n_heads, self.head_dim).transpose(1, 2)
        ...
        h = self._apply_norm(self.norm1, h + out)
        h = self._apply_norm(self.norm2, h + self.ff(h))
        return h


class EdgeBiasedGraphAttentionEncoder(nn.Module):
    """
    Field-level encoder with pairwise edge bias.

    edge_features: [B, M, M, E]
    edge_bias_mlp(edge_features): [B, M, M, H]
    The bias is added as attention_score(i, j) += bias_head(i, j).
    """
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            EdgeBiasedMultiHeadAttentionLayer(
                n_heads=n_heads,
                embed_dim=embed_dim,
    ...
    def forward(self, h, edge_features=None):
        edge_bias = None
        if edge_features is not None:
            # [B, M, M, H] -> [B, H, M, M]
            edge_bias = self.edge_bias_mlp(edge_features).permute(0, 3, 1, 2).contiguous()
...
class AttentionModel(nn.Module):
    ...
    def __init__(
        ...
        # Global encoder over [depot, field_1, ..., field_N] with additive edge bias:
        # attention_score(i, j) += MLP(edge_features(i, j)).
        self.field_edge_feature_dim = 13
        self.field_encoder = EdgeBiasedGraphAttentionEncoder(
            n_heads=n_heads,
            embed_dim=embedding_dim,
            n_layers=n_encode_layers,
            edge_dim=self.field_edge_feature_dim,
    ...
    def _encode_input(self, depot, templates):
        ...
        # Edge features make field-level self-attention transition-aware.
        encoder_input = torch.cat((depot_embedding, field_init), dim=1)
        edge_features = self._make_field_edge_features(depot, templates)
        field_embeddings, _ = self.field_encoder(encoder_input, edge_features=edge_features)

        return field_embeddings

    def _make_field_edge_features(self, depot, templates):
        """
        Pairwise edge features for field-level encoder.

        Returns:
            [B, N + 1, N + 1, 13]
            index 0 is depot, indices 1..N are fields.

        Features for directed pair i -> j:
            0  min transition distance i -> j
            1  mean transition distance i -> j
            2  min reverse transition distance j -> i
            3  mean reverse transition distance j -> i
            4  center distance
            5  min coverage of destination j
            6  mean coverage of destination j
            7  max coverage of destination j
            8  min return distance from destination j to depot
            9  mean return distance from destination j to depot
            10 is source depot
            11 is destination depot
            12 is self edge
        """
        ...
        M = N + 1
        edge = torch.zeros(B, M, M, self.field_edge_feature_dim, device=device, dtype=dtype)

        # Field -> field directed transition statistics.
        # dist[b, i, j, ki, kj] = ||exit(i, ki) - entry(j, kj)||
        dist_ff = (
            exit_[:, :, None, :, None, :] - entry[:, None, :, None, :, :]
        ).norm(p=2, dim=-1)  # [B, N, N, K, K]
        min_ff = dist_ff.amin(dim=(3, 4))
        mean_ff = dist_ff.mean(dim=(3, 4))

        edge[:, 1:, 1:, 0] = min_ff
        edge[:, 1:, 1:, 1] = mean_ff
        edge[:, 1:, 1:, 2] = min_ff.transpose(1, 2)
        edge[:, 1:, 1:, 3] = mean_ff.transpose(1, 2)

        # Depot -> field and field -> depot.
        depot_to_entry = (entry - depot[:, None, None, :]).norm(p=2, dim=-1)  # [B, N, K]
        exit_to_depot = (exit_ - depot[:, None, None, :]).norm(p=2, dim=-1)   # [B, N, K]

        edge[:, 0, 1:, 0] = depot_to_entry.min(dim=2).values
        edge[:, 0, 1:, 1] = depot_to_entry.mean(dim=2)
        edge[:, 1:, 0, 0] = exit_to_depot.min(dim=2).values
        edge[:, 1:, 0, 1] = exit_to_depot.mean(dim=2)

        # Reverse stats for depot-related edges.
        edge[:, 0, 1:, 2] = exit_to_depot.min(dim=2).values
        edge[:, 0, 1:, 3] = exit_to_depot.mean(dim=2)
        edge[:, 1:, 0, 2] = depot_to_entry.min(dim=2).values
        edge[:, 1:, 0, 3] = depot_to_entry.mean(dim=2)

        # Centers: field center is mean of all entry/exit points.
        field_center = torch.cat((entry, exit_), dim=2).mean(dim=2)  # [B, N, 2]
        centers = torch.cat((depot[:, None, :], field_center), dim=1)  # [B, M, 2]
        edge[..., 4] = (centers[:, :, None, :] - centers[:, None, :, :]).norm(p=2, dim=-1)

        # Destination coverage features, zero for depot destination.
        min_cov = coverage.min(dim=2).values
        mean_cov = coverage.mean(dim=2)
        max_cov = coverage.max(dim=2).values
        edge[:, :, 1:, 5] = min_cov[:, None, :]
        edge[:, :, 1:, 6] = mean_cov[:, None, :]
        edge[:, :, 1:, 7] = max_cov[:, None, :]

        # Destination return-to-depot features.
        edge[:, :, 1:, 8] = exit_to_depot.min(dim=2).values[:, None, :]
        edge[:, :, 1:, 9] = exit_to_depot.mean(dim=2)[:, None, :]
