...
# TensorBoard сейчас не используется.
...
class Config:
    ...
    # Data
    # -------------------------
    # problem нужен для чекпоинтов и graph_size
    problem = "tsp"
    graph_size = 20
    ...
    # -------------------------
    output_dir = "outputs"
    run_name = "=+2ENCODER_1ATTENTION20runtestdeportGTSP20"
    checkpoint_epochs = 25

    # загрузить готовую модель / продолжить обучение
    load_path = None
    resume = None
...
def run(opts):
    ...
    # Создаём/очищаем JSONL-лог для нового запуска.
    # При resume лучше не очищать старый лог.
    if opts.log_jsonl and opts.resume is None:
    ...
    if not opts.no_tensorboard:
        # TbLogger должен быть раскомментирован в import-ах, если решишь включить TensorBoard.
        tb_logger = TbLogger(
            os.path.join(
                opts.log_dir,
                f"{opts.problem}_{opts.graph_size}",
    ...
    if opts.eval_only:
        avg_cost = validate(model, val_dataset, opts, epoch=None, log=True)
