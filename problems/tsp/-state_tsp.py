...
class StateTSP(NamedTuple):
    ...
    # Dynamic state
    prev_a: torch.Tensor           # [B, 1], 0 = depot, 1..N*K = selected action
    prev_field: torch.Tensor       # [B, 1], 0 = depot, 1..N = field index for encoded fields
    visited_fields: torch.Tensor   # [B, 1, N], True = field already covered
    lengths: torch.Tensor          # [B, 1]
    cur_coord: torch.Tensor        # [B, 1, 2], current physical exit point
    i: torch.Tensor
    ...
    def update(self, selected_action):
        ...
        selected_action:
            [B]
            1..N*K, where:
                field_id    = (selected_action - 1) // K, 0..N-1
                template_id = (selected_action - 1) % K,  0..K-1
        """
    ...
    def get_mask(self):
        """
        Compatibility method with the flat decoder version.

        Returns mask over actions:
            [B, 1, 1 + N*K]

        0/depot is always forbidden.
        All templates of visited fields are forbidden.
        """
