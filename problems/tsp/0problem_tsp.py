...
class TSP(object):
    ...
    def get_costs(dataset, pi):
        ...
        dataset:
            {
                'templates': [batch, n_fields, n_templates, 4],
                'depot':     [batch, 2]
            }

        templates[..., 0:2] = input point  шаблона
        templates[..., 2:4] = output point шаблона

        pi:
            [batch, n_fields]

            Значения pi лежат в 1..n_fields*n_templates,
            потому что:
                0       = depot
        ...
        # pi: 1..N*K
        selected = pi - 1  # 0..N*K-1

        field_ids = selected // n_templates      # [B, N]
        template_ids = selected % n_templates    # [B, N]
        ...
        chosen_in = chosen_templates[..., 0:2]   # [B, N, 2]
        chosen_out = chosen_templates[..., 2:4]  # [B, N, 2]
        ...
        # output текущего шаблона -> input следующего шаблона
        between_templates = (
            chosen_in[:, 1:] - chosen_out[:, :-1]
        ).norm(p=2, dim=2).sum(1)
