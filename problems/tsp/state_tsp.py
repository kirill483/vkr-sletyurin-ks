...
class StateTSP(NamedTuple):
    ...
    Additional field:
        prev_soft_exit:
            soft expected exit point of the previously selected field.
            Shape: [B, 1, 2]
    """
    ...
    # Optional compatibility tensors.
    ...
    prev_soft_exit: torch.Tensor    # [B, 1, 2], soft expected exit of previous selected field
    visited_fields: torch.Tensor    # [B, 1, N], True = field already selected
    i: torch.Tensor                 # scalar step counter

    # Compatibility fields with older StateTSP interface.
    ...
    def initialize(input, node_in=None, node_out=None):
        ...
        if node_in is None or node_out is None:
            candidate_in = templates[..., 0:2].reshape(
                batch_size,
                n_fields * n_templates,
                2,
            )
            candidate_out = templates[..., 2:4].reshape(
                batch_size,
                n_fields * n_templates,
                2,
            )
        ...
        return StateTSP(
            ...
            first_field=torch.zeros(
                batch_size,
                1,
                dtype=torch.long,
                device=templates.device,
            ),
            prev_soft_exit=depot_coord,
    ...
    def update_field(self, selected_field, soft_exit=None):
        """
        selected_field:
            [B], values in 0..N-1

        soft_exit:
            [B, 1, 2], soft expected exit of selected field.
            If None, previous soft exit is kept.
        """
