...
class TSP(object):
    ...
    def get_costs(dataset, pi):
        ...
        dataset:
            {
                'templates': [B, N, K, 4] or [B, N, K, 5],
                'depot':     [B, 2]
            }

        templates[..., 0:2] = entry point
        templates[..., 2:4] = exit point
        templates[..., 4]   = coverage length, optional
        ...
        batch_size, n_fields, n_templates, feature_dim = templates.size()
        assert feature_dim in (4, 5)
        ...
        assert (
            torch.arange(n_fields, device=pi.device, dtype=field_ids.dtype)
            .view(1, -1)
            .expand_as(field_ids)
            == field_ids.sort(1)[0]
        ...
        chosen_templates = templates[batch_idx, field_ids, template_ids]  # [B, N, 4 or 5]
        ...
        if feature_dim == 5:
            ...
            chosen_coverage = torch.zeros(batch_size, n_fields, dtype=templates.dtype, device=templates.device)
