...
def train_epoch(
    ...
    distance_alpha = None
    if hasattr(model, "distance_logit_scale"):
        distance_alpha = torch.nn.functional.softplus(
            model.distance_logit_scale
        ).item()

        print("distance alpha:", distance_alpha)
    ...
    # Если хочешь логировать candidate_mean/baseline_mean,
    # лучше добавить JSONL прямо внутрь RolloutBaseline.epoch_callback.
